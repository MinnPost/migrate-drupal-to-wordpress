...
DROP TABLE IF EXISTS acc_users_post_count;
DROP TABLE IF EXISTS acc_users_add_commenters;
...
REPLACE INTO acc_wp_posts (
	...
	LEFT OUTER JOIN url_alias a
		...
		#WHERE n.type IN ( 'article', 'article_full', 'audio', 'event', 'newsletter', 'video' );
		WHERE n.type IN ( 'article', 'article_full' );
...
/* Set site email */
UPDATE acc_wp_options SET option_value = ( SELECT value FROM variable WHERE name='site_mail') WHERE option_name = 'jstegall@minnpost.com';
